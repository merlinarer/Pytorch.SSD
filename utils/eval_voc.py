...
class VOCMap:
    ...
    def do_python_eval(self, box_list, img_list, use_07=True):
        ...
        for i, cls in enumerate(labelmap):
            rec, prec, ap = self.voc_eval(
                box_list, img_list, self.annopath, self.imgsetpath.format(self.set_type), cls, cachedir,
    ...
    def voc_eval(self, box_list, img_list,
                 annopath,
                 imagesetfile,
                 classname,
                 cachedir,
    ...
    """
        ...
        # read dets
        class_dict = {k: v for v, k in enumerate(labelmap)}
        clsid = class_dict[classname] + 1
        box_cls = box_list[clsid]
        if len(box_cls) > 1:
            ...
            for ind, imname in enumerate(img_list):
                if len(box_cls[ind]) > 0:
                    for k in range(len(box_cls[ind])):
                        image_ids.append(imname.split('.')[0])
                        confidence.append(box_cls[ind][k, -1])
                        BB.append(box_cls[ind][k, :-1])
    ...
    def test_net(self, net, dataloader):
        ...
        for img, _, _, imginfo in dataloader:
            ...
            imgname = [info['name'] for info in imginfo]
        ...
        self.evaluate_detections(all_boxes, img_list)

    def evaluate_detections(self, box_list, img_list):
        self.do_python_eval(box_list, img_list)
