...
from modeling.utils.coder import encode, decode, encode_xyxy
...
class SSDHead(nn.Module):
    ...
    def match_single_img(self, flat_anchors, gt_bboxes, gt_labels, batch_size=4):
        ...
        :param flat_anchors: Tensor: num_anchors, 4  xyxy [0,1]
        ...
        if len(pos_idx) > 0:
            pos_bbox_targets = encode_xyxy(pos_gt_bboxes, pos_bboxs, self.variance)
    ...
    def loss(self, outs, gt):
        ...
        multi_level_anchors = self.anchor_generator.grid_anchors(image_size=300) #xywh [0,1]
        ...
        return sum(loss_cla)[0], sum(loss_bbox)
